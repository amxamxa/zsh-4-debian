...
############################################### --- ENDE
# Define a custom function to log errors
# log_error.txt
# in gleichem Terminal: less -F ~/error-log.txt
# in separaten Terminal: tail -f ~/error-log.txt
##log_error_basic() {
## if [ "$?" -ne 0 ]; then
##   local last_command=$(fc -ln -1)
##   # für bash: 
##	 #für ZSH $(fc -ln -1) 
##          echo "$(date) - Command: $BASH_COMMAND" >> "$ZDOTDIR/log-error/log_error.txt"
##   echo "$(date) - Command: $last_command" >> "$ZDOTDIR/log-error/log_error.txt"
##   echo "$(cat /dev/stdin)" >> "$(ZDOTDIR)/log_error.txt"
## fi
##}
# Definiere eine benutzerdefinierte Funktion zum Protokollieren von Fehlern
log_error_extended() {
  if [ "$?" -ne 0 ]; then
    local last_command=$(fc -ln -1)
    local exit_code=$?
    #local output=$(eval "$last_command")
    local shell_output=$_  
    local username=$(whoami)
    echo "$(date) - User: $username - zsh Shell Command: $shell_output - Exit code: $exit_code - zsh shell Output:" >> "$ZDOTDIR/log_error/log_error.txt"
  fi
}
...
#######apt install zoxide: _z_cd, z , zi, za, zq, zqi, zr zri#####################################
