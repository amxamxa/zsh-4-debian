...
#inv="\033[38;2;232;197;54m\033[48;2;128;87;0m" #gelb
...
alias cat='batcat --force-colorization --number --terminal-width 80 --wrap=auto --theme=ansi'
...
alias ZRCg='gedit "$ZDOTDIR/.zshrc" & && source "$ZDOTDIR/.zshrc"'
alias ZALI='nano $ZDOTDIR/aliases.zsh && source $ZDOTDIR/aliases.zsh && echo -e "\n\t${inv}source $ZDOTDIR/aliases.zsh erfolgreich!${res}\n"'
alias ZALIg='gedit "$ZDOTDIR/aliases.zsh" && source "$ZDOTDIR/aliases.zsh" && echo -e "\n\t${inv}source $ZDOTDIR/aliases.zsh erfolgreich!${res}\n"'
alias ZSAVE='cp "$ZDOTDIR/.zshrc" "$ZDOTDIR/bkp/$(date +'%F').zshrc.zsh" && cp "$HOME/.zshenv" "$ZDOTDIR/bkp/$(date +'%F').zshenv.zsh" && cp "$ZDOTDIR/aliases.zsh" "$ZDOTDIR/bkp/$(date +'%F')_aliases.zsh"'

alias wh_fuck='dpkg -L' # alle Dateien finden, die mit 'apt install' installiert wurde

#alias WINsize='echo -e "\t${inv}2x klicken, zeigt Parameter des Windows an!${res}\n" && "xprop | grep WM_CLASS\(STRING\) && xwininfo"'  # größe des Windows ermnittln
alias D='echo -e "\n\t${inv}Zeige das aktuelle Datum${res}\n" && date "+%A, %B %-d,%Y"'
...
alias la='echo -e "\t${inv}ls -plah: Zeigt alles${res}\n" && ls -plah'
alias lt='echo -e "\t${inv}ls -AsltpGH: Liste, sortiert nach Änderungsdatum (älteste zuerst)${res}\n" && ls -AsltpGH'
alias lx='echo -e "\t${inv}ls -AXlGh: Liste sortiert nach File-EextensionÄnderungsdatum${res}\n" && ls -AXlGhp' 
