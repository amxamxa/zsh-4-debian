...
log_error() {
 if [ "$?" -ne 0 ]; then
   ...
   # für bash:  echo "$(date) - Command: $BASH_COMMAND" >> "$ZDOTDIR/log_error.txt"
   echo "$(date) - Command: $last_command" >> "$ZDOTDIR/log_error.txt"
   echo "$(cat /dev/stdin)" >> "$ZDOTDIR/log_error.txt"
 fi
}
# Call the log_error function from the precmd function
precmd() {
  log_error
}
